...
In the example below, query_results.parquet already exists and so no query will be executed. Instead, the .parquet file will be read in using `arrow::read_parquet()`
```{r example}
results <- getAzureResults(
  SQLscript = query,
  output = "data/query_results.parquet"
)
results
```
